...
const AdminBookedRooms = () => {
  ...
  const [allReservedRooms, setAllReservedRooms] = useState(null)
  ...
  const handleChangeReservation = (text) => {
    ...
    if(text.target.value == 'UNRESERVE'){
      ...
      setReservationType('UNRESERVED')
      setCancel('true')
    }else if(text.target.value == 'CHECKOUT'){
      console.log('out')
      setReservationType('UNRESERVED')
      setCancel('false')
    }
  ...
  const columns = [
    ...
    { 
     ...
     flex:1,
      renderCell: (params) => {
        const item = reservedRooms?.find(s => s.room_id == params.row?.id)
        return <span>{Number(params.row?.amount * item?.duration).toLocaleString(undefined, {maximumFractionDigits:2})}</span>
      } 
    },
    { 
      field: "createdAt", 
